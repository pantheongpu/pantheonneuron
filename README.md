...
(a real backward pass and optimiser step — 23.25 train-steps/s on
2026-08-27, for a 2-layer 1024-wide MLP at batch 16; the pinned
`transformer_train_step` problem is larger and runs at 3.65, see the table
below), the
...
**Measured** is the median of three trn1.2xlarge hosts, 300 s × 3 repeats
each, from the 2026-09-21 pass
([`summary-trn1.json`](data/publication-2026-09-21/summary-trn1.json)), in
the registry's own unit. It is generated by
`tools/readme_measurements.py --write`, and a test fails if it drifts: two
figures here were typed from earlier single runs and sat 9% and 5.5% from
the published dataset with nothing to say so.

| Workload | Status | Measured | Score source |
|---|---|--:|---|
| `baseline_metrics` | ✅ telemetry only, no load | — | — |
| `memory_read` | ✅ scored from its declared source | 272.98 GB/s | `neuron-profile` |
| `memory_write` | ✅ scored from its declared source, 4 GiB pin, 8192-wide stores | 274.82 GB/s | `neuron-profile` |
| `memory_read_agg` | ✅ loops start together (barrier), 100% overlap | 543.74 GB/s | workload |
| `memory_write_agg` | ✅ loops start together (barrier), 8192-wide stores | 537.25 GB/s | workload |
| `tensor_virus` | ✅ at the pinned 8192³, coalesced tiling, 8 accumulators | 78.60 TFLOPS | `neuron-monitor` |
| `int_virus` | ✅ at the pinned 8192³, uint8, coalesced tiling | 78.84 TOPS | `neuron-monitor` |
| `pulse_virus` | ✅ at the pinned 8192³, 50% duty, coalesced tiling | 39.94 TFLOPS | `neuron-monitor` |
| `omni_virus` | ✅ at the pinned 8192³ | 54.01 TFLOPS | `neuron-monitor` |
| `transformer_virus` | ✅ realistic instruction mix | 53.25 TFLOPS | `neuron-monitor` |
| `graph_replay` | ✅ per-period completion tallies, cv 0.006 | 3,038.0 graph-steps/s | `neuron-monitor` |
| `allocation_fragmentation` | ✅ | 2,397.2 allocation-events/s | workload |
| `llm_prefill` | ✅ pre-normalised, no NaN | 3,811.7 prompt-tokens/s | workload |
| `llm_decode` | ✅ q, k, v and o per token since 2026-09-11 | 17.54 tokens/s | workload |
| `kv_cache_churn` | ✅ memory-bound at last | 97,348.4 cache-updates/s | workload |
| `fused_attention` | ✅ | 6,048.4 attention-tiles/s | workload |
| `quantized_gemm` | ✅ | 1.845e+13 quantized-ops/s | workload |
| `moe_router` | ✅ balanced dispatch | 255,242.3 routed-tokens/s | workload |
| `speculative_decode` | ✅ verifies through the target model | 73.80 verified-tokens/s | workload |
| `rag_embedding` | ✅ 12-layer encoder | 854.46 embedding-vectors/s | workload |
| `vision_encoder` | ✅ 12-layer ViT | 58,155.4 image-tiles/s | workload |
| `transformer_train_step` | ✅ skips on Inferentia | 3.65 train-steps/s | workload |
| `pcie_bandwidth` | ⚠️ explained: the pin sits past a transfer-size cliff | 3.68 GB/s | workload |
| `serving_mix` | ⚠️ Score quantised to one request per 32 decode steps | 2.48 requests/s | workload |
...
| [Checks that pass by accident](docs/checks_that_pass_by_accident.md) | 28 of them, the four shapes they keep taking, and why a green run is evidence about the checks rather than the code |
